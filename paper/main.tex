...
\textbf{Backbones:} Reflex (System~1): Qwen2.5-0.5B-Instruct with Safe-to-Say LoRA (rank 8, $\alpha{=}16$, target layers 8--24); Main (System~2): Qwen2.5-7B-Instruct. Shared tokenizer; temperature 0.7 for System~1 bridges, greedy for System~2. Hardware: 8$\times$ NVIDIA RTX 3090 (24GB); System~1 on GPU~0, System~2 across GPUs~5--7. Memory budget $B_{\max}{=}900$ tokens; homeostasis $\theta{=}0.5$, $D_{\max}{=}0.3$. PsyAgent/Sophia/CAIM baselines are re-implemented under the same backbone/decoding/corpus; unavailable model weights are replaced by prompting-only equivalents with matched generation budget (Appendix~\ref{app:repro-checklist}). Multilingual results in Appendix~\ref{app:generalization}.
...
\textbf{NLI stitcher.} The DeBERTa-v3-base entailment classifier is evaluated on 13 held-out bridge/response pairs spanning entailment, neutral, and contradiction categories; we additionally evaluate on an expanded 41-pair set covering five commitment types (Appendix~\ref{app:stitching-conflicts}).
...
On an expanded 41-pair validation set covering memory-recall, commitment, factual, and implicature categories, overall accuracy is \textbf{73.2\%} (95\% CI $[58.5, 85.4]$); entailment and contradiction detection are robust (100\% and 91.7\%) while neutral is harder (16.7\%), consistent with standard NLI benchmarks. Full confusion matrix and per-type breakdown in Appendix~\ref{app:stitching-conflicts}.
...
Our NLI stitcher validation uses a 13-pair curated qualitative probe (85.7\%) and an expanded 41-pair set (73.2\%; CI $[58.5, 85.4]$); broader coverage across conflict types and languages remains future work, and we commit to releasing a larger validated set ($>$100 pairs) with the codebase. Our Trait--State formulation covers Big Five and defense mechanisms; richer constructs and broader multilingual coverage are open directions.
...
See the Ethics Statement (before the bibliography in the main text) for a full discussion of para-social attachment, privacy, and adversarial risks.
...
\paragraph{Expanded validation (41 pairs).}
To address concerns about the small held-out set, we additionally evaluated the classifier on an expanded set of 41 bridge/response pairs covering five categories: memory recall, commitment/promise, factual, implicature-safe, and implicature-contradiction. The expanded set yields \textbf{73.2\%} overall accuracy (30/41; 95\% CI $[58.5, 85.4]$, bootstrap $B{=}10{,}000$), with class-level breakdown in Table~\ref{tab:nli-validation-expanded}. Entailment and contradiction detection are robust (100\% and 91.7\% respectively); neutral is the hard class (16.7\%), consistent with prior NLI literature showing neutral is the most ambiguous category. Failures in the neutral class are dominated by the model predicting entailment---interpreting a topically unrelated response as entailed by the bridge---which motivates Safe-to-Say as the primary commitment-prevention mechanism rather than post-hoc NLI repair. We commit to releasing a larger validated set ($>$100 pairs covering multilingual and implicit commitment types) alongside the codebase.

\begin{table*}[t]
  \caption{NLI stitcher held-out validation: DeBERTa-v3-base on 13 bridge/response pairs (model: cross-encoder/nli-deberta-v3-base; device: RTX~3090).}
  \label{tab:nli-validation}
  \centering
  \setlength{\tabcolsep}{4pt}
  \begin{tabular}{p{0.24\textwidth} ccc p{0.35\textwidth}}
    \toprule
    Class & Cases & Correct & Accuracy (\%) & Notes \\
    \midrule
    Entailment & 6 & 6 & 100.0 & Bridge consistent with response \\
    Neutral & 3 & 2 & 66.7 & One case: ambiguous context \\
    Contradiction & 4 & 3 & 75.0 & One case: implicit implicature \\
    \midrule
    \textbf{Overall} & \textbf{13} & \textbf{11} & \textbf{85.7} & \\
    \bottomrule
  \end{tabular}
\end{table*}

\begin{table*}[t]
  \caption{NLI stitcher expanded validation (41 pairs, 5 categories). Per-class accuracy; neutral is the hardest class as in standard NLI benchmarks. Confusion: entailment row $[17,0,0]$; neutral $[7,2,3]$; contradiction $[1,0,11]$. CI: 95\% bootstrap $[58.5, 85.4]$.}
  \label{tab:nli-validation-expanded}
  \centering
  \setlength{\tabcolsep}{4pt}
  \begin{tabular}{lcccl}
    \toprule
    Class & Cases & Correct & Acc.\ (\%) & Primary failure mode \\
    \midrule
    Entailment & 17 & 17 & 100.0 & None \\
    Neutral    & 12 &  2 &  16.7 & Model predicts entailment (off-topic treated as consistent) \\
    Contradiction & 12 & 11 & 91.7 & One implicit implicature case \\
    \midrule
    \textbf{Overall} & \textbf{41} & \textbf{30} & \textbf{73.2} & 95\% CI $[58.5, 85.4]$ \\
    \bottomrule
  \end{tabular}
\end{table*}
...
On the full 51-case set, TEMPO achieves \textbf{80.4\%} accuracy (41/51; recall 86.3\%, state alignment 90.2\%) using Qwen2.5-7B on RTX~3090---substantially better than a preliminary mock-inference estimate of 29.4\% reported in an earlier draft.
...
(iv) a permanent DOI-linked archive (Zenodo) with the exact commit hash and corpus snapshot used for all paper results.
...
In the final version, we will include (a) a single compressed snapshot of the retrieval corpus (or a script that deterministically reconstructs it from public sources) and (b) the exact commit hash from the code repository used to generate all results.

\begin{table}[h]
  \caption{Artifact manifest for reproducibility. Camera-ready version will include exact corpus hashes and commit IDs.}
  ...
  \\[2pt]
  \raggedright\footnotesize Anonymous repository: \url{https://anonymous.4open.science/r/TEMPO-2FBF}. Permanent DOI and commit hash to be added at camera-ready.
\end{table}

\paragraph{Full reproduction commands.}
The following commands reproduce the main results. All commands assume the \texttt{realm} conda environment (Python 3.10, PyTorch 2.1, Transformers 4.40, PEFT 0.10) and are run from the repository root.

\begin{Verbatim}[fontsize=\footnotesize]
# Main evaluation (Tables 2+3): TTFT, PNH-10/51, NLI stitcher
conda run -n realm python experiments/tempo_full_eval.py \
  --seed 42 --s1 Qwen/Qwen2.5-0.5B-Instruct \
  --s2 Qwen/Qwen2.5-7B-Instruct \
  --lora models/safe_to_say_lora \
  2>&1 | tee results/full_eval_seed42.log

# 14B scale experiment (Appendix I.16)
# Set MODEL_DIR to your 14B model location
conda run -n realm python experiments/run_14b_scale_experiment.py \
  --s2 ${MODEL_DIR}/Qwen2.5-14B-Instruct \
  --seed 42

# Human evaluation statistics (Appendix G)
conda run -n realm python experiments/compute_full_stats.py

# LoRA training (Safe-to-Say)
...
\noindent\textbf{Environment.} LoRA config: \texttt{r=8, lora\_alpha=16, target\_modules=[\textquotesingle{}q\_proj\textquotesingle{},\textquotesingle{}v\_proj\textquotesingle{}], lora\_dropout=0.05}. Tokenizer: same as base model (\texttt{Qwen2.5-0.5B-Instruct}, fast tokenizer). HuggingFace mirror: \texttt{https://hf-mirror.com}. Hardware: RTX~3090 (24GB) $\times$ 1--3 depending on model size.
...
\begin{table}[h]
  \caption{Baseline implementation checklist. All baselines share the same backbone, retrieval corpus, and random seeds unless noted.}
